...
from bookstore.forms import RegistrationForm, LoginForm, ProductForm, CommentForm, LikeOrDisLikeForm
from bookstore.models import RegistrationModel, ProductModel, CommentModel, LikeOrDisLikeModel,TransactionModel
from bookstore.service import getAllProducts
...
def registration(request):
    ...
    if request.method == "POST":
        ...
        if registrationForm.is_valid():
            ...
            regForm.name = registrationForm.cleaned_data["name"]
            regForm.email = registrationForm.cleaned_data["email"]
            regForm.mobile = registrationForm.cleaned_data["mobile"]
            regForm.address = registrationForm.cleaned_data["address"]
            regForm.username = registrationForm.cleaned_data["username"]
            regForm.password = registrationForm.cleaned_data["password"]

            user = RegistrationModel.objects.filter(username=regForm.username).first()
...
def getuserinfo(request):
    return render(request, "users.html", {"users":RegistrationModel.objects.filter(username=request.GET["userid"])})